...
### 待做（按优先级，均在定位内）
- ⬜ **P1 软封杀早期预警**（★护城河）：五前兆信号——验证码触发率↑ / 响应字段完整度↓ / 延迟分布漂移 / 蜜罐命中 / 成功率下降斜率 → **被封前**降速换号而非等 403。是现有 `canary` 的自然进化。**理由**：Mirage 是全链路唯一同时看到"注入了什么+发出去什么+回来什么"的位置——反检测浏览器只管指纹、代理商只管 IP、打码商只管过码，谁都占不到这个观测位。
