...
- ✅ **P1 统一设备画像 + 自洽校验** → `scripts/device_profile.py` + `mirage profile <种子>`。同种子→同画像（跨会话稳定/多账号隔离）；BUNDLES 是**真实自洽套餐**（UA↔platform↔GPU↔屏幕配对，不逐项随机拼，避免"Windows UA + Apple GPU"致命矛盾）；`check()` 出厂校验。实测 200/200 生成画像零矛盾、5 类人造矛盾全被抓。
- ✅ **P1 随机分布 uniform → 对数正态**：`human_delay()` 单一真相源；注入模板 L2/L4 同步改（`expr * random.lognormvariate(0,0.45)`）；L5 兜底的直线匀速也改缓入缓出；L5 锚点正则放宽以**兼容已部署的老 uniform 环境**。实测偏度 **-0.01(平顶矩形) → +1.43(右偏)**，中位数仍锚 base。
- ✅ **P2 种子化确定性 canvas/audio 噪声**（并入 device_profile）：LCG 由 noiseSeed 驱动，**同 profile 恒定**；拦 `toDataURL`/`toBlob`/`getImageData` 三出口（只拦一个会被另一出口对拍抓出）。这是替代"每次随机噪声"——后者在 2026 已反成判伪证据。
- ✅ **P2 验证码回调钩子** → `MirageLoop(on_captcha=fn)`。**Mirage 不破解验证码**（红海+偏离定位），只做"检测→挂钩→回填时机拟人化"。安全默认：无回调/返回 False/回调抛错/**回调声称已解决但复检信号仍在** → 一律停手；仅"已解决且信号消失"才继续，且插入拟人停顿（秒续本身是机器特征）。dry-run 不触发回调。实测 7 路径全过。
